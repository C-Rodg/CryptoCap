...
class App extends Component {
	...
	handleAddAlert = alertObject => {
		...
		const myNewPriceAlerts = this.state.priceAlerts.filter(alert => {
			if (
				...
				console.log("SAME KIND OF NOTIFICATION FOUND");
				return false;
			}
			return true;
		});
	...
	checkForPriceAlert = cryptoInfoArray => {
			...
			if (specifiedCrypto) {
				if (
					...
					notificationArray.push(
						`${specifiedCrypto.id} is below ${this.formatPriceAlert(
							priceAlert.priceUSD,
							type,
							locale
						)}`
					...
					notificationArray.push(
						`${specifiedCrypto.id} is above ${this.formatPriceAlert(
							priceAlert.priceUSD,
							type,
							locale
						)}`
		...
		console.log("new price alerts!");
		console.log(newPriceAlerts);
	...
	// Format price alert based off of selected currency
	formatPriceAlert = (num, type, locale) => {
		let val = num;
		if (
			type !== "USD" &&
			this.state.exchangeRates &&
			this.state.exchangeRates.rates &&
			this.state.exchangeRates.rates[type]
		) {
			val = this.state.exchangeRates.rates[type] * num;
		}
